...
vec3 Camera::ray_color(const ray& r, int depth, const geometry& world) const
{
	...
	ray scattered;
	vec3 attenuation;
	vec3 emission_color = g_point.mat->emitted(g_point.u, g_point.v, g_point.p);

	gp_data geo_data = { g_point.p, g_point.normal, g_point.front_face, g_point.u, g_point.v };


	if (!g_point.mat->scatter(r, geo_data, attenuation, scattered))
		return emission_color;

	vec3 scatter_color = attenuation * ray_color(scattered, depth - 1, world);
